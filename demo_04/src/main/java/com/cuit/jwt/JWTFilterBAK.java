...
public class JWTFilterBAK extends BasicHttpAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        System.out.println("isAccessAllower");
        //判断请求的请求头是否带上 "Token"
...
//            onLoginFail(response,e.getMessage());
    ...
    //登录失败时默认返回 401 状态码
    public static void onLoginFail(ServletResponse response,String msg) throws IOException {
        HttpServletResponse httpResponse = (HttpServletResponse) response;
        httpResponse.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
        httpResponse.getWriter().write("dasdasd-->"+msg);
    }
}
